...
if st.session_state.audio_recorded:
    ...
    model_name = st.selectbox("Whisper 모델 선택", ["tiny", "base", "small"], index=2)
...
- **포트폴리오**: 금융권 면접에서 Streamlit 앱으로 음성 처리 기술 시연.
""")

# 사이드바: 시스템 정보 및 설정
with st.sidebar:
    st.header("설정 및 정보")
    st.write(f"**Whisper 모델**: {model_name if 'model_name' in locals() else ''}")

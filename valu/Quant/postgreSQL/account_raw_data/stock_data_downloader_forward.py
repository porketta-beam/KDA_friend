...
def get_latest_date_for_ticker(conn, ticker):
    """
    account_raw_data 테이블에서 해당 티커의 가장 최신 날짜를 조회합니다.
    """
    cursor = conn.cursor()
    query = """
        SELECT MAX(d.date)
...
def main():
    ...
    default_start_dt = datetime.now() - timedelta(days=380)

    for i, ticker in enumerate(tickers, 1):
        logger.info("티커 %d/%d 처리 시작: %s", i, len(tickers), ticker)
        last_date = get_latest_date_for_ticker(conn, ticker)
        if last_date:
            start_dt = last_date + timedelta(days=1)
        else:
            start_dt = default_start_dt
        start_str = start_dt.strftime('%Y%m%d')
        logger.info("티커 %s: %s부터 수집", ticker, start_str)

        data_df = download_and_reshape_data(ticker, start_str, end_date)
